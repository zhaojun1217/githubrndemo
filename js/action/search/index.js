...
import * as ArrayUtil from 'eslint/lib/linter/rule-fixer';
...
export function onSearch(inputKey, pageSize, token, favoriteDao, popularKeys, callBack) {
    return dispatch => {
        ...
        fetch(genFetchUrl(inputKey)).then(response => { // 如果任务取消，则不处理
            return hasCancel(token) ? null : response.json();
        }).then(responseData => {
            if (hasCancel(token, true)) { // 如果任务取消，则不处理
                console.log('user cancel');
                return;
            }
            if (!responseData || !responseData.items || responseData.items.length === 0) { // 数据不存在
                dispatch({type: Types.SEARCH_FAIL, message: `没找到${inputKey}的项目`});
                doCallBack(callBack, `没找到${inputKey}的项目`);
                return;
            }
